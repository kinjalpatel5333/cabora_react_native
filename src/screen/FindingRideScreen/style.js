import {StyleSheet} from 'react-native';

export default function createStyles(colors) {
  return StyleSheet.create({
    root: {
      flex: 1,
      justifyContent: 'flex-end',
      backgroundColor: 'transparent',
    },
    backdrop: {
      ...StyleSheet.absoluteFillObject,
    },
    backBtn: {
      position: 'absolute',
      left: 16,
      zIndex: 30,
      elevation: 30,
      width: 44,
      height: 44,
      borderRadius: 22,
      backgroundColor: colors.card,
      alignItems: 'center',
      justifyContent: 'center',
      shadowColor: colors.navy[900],
      shadowOpacity: 0.12,
      shadowRadius: 8,
      shadowOffset: {width: 0, height: 2},
    },
    radarArea: {
      position: 'absolute',
      left: 0,
      right: 0,
      top: '8%',
      height: '42%',
      alignItems: 'center',
      justifyContent: 'center',
    },
    radarPulse: {
      position: 'absolute',
      width: 120,
      height: 120,
      borderRadius: 60,
      borderWidth: 2,
      borderColor: colors.orange[400],
      backgroundColor: colors.alpha.orangeBrand12,
    },
    radarCore: {
      alignItems: 'center',
      justifyContent: 'center',
      zIndex: 2,
      shadowColor: colors.orange[600],
      shadowOpacity: 0.35,
      shadowRadius: 10,
      shadowOffset: {width: 0, height: 4},
      elevation: 6,
    },
    radarPin: {
      marginBottom: -4,
    },
    vehiclePin: {
      position: 'absolute',
      width: 40,
      height: 40,
      borderRadius: 20,
      backgroundColor: colors.card,
      alignItems: 'center',
      justifyContent: 'center',
      shadowColor: colors.navy[900],
      shadowOpacity: 0.14,
      shadowRadius: 6,
      shadowOffset: {width: 0, height: 2},
      elevation: 4,
      zIndex: 3,
    },
    sheet: {
      backgroundColor: colors.card,
      borderTopLeftRadius: 28,
      borderTopRightRadius: 28,
      paddingHorizontal: 20,
      paddingTop: 10,
      overflow: 'hidden',
      shadowColor: colors.navy[900],
      shadowOpacity: 0.14,
      shadowRadius: 18,
      shadowOffset: {width: 0, height: -4},
      elevation: 22,
    },
    grabber: {
      alignSelf: 'center',
      width: 40,
      height: 4,
      borderRadius: 2,
      backgroundColor: colors.border,
      marginBottom: 16,
    },
    grabberHit: {
      alignItems: 'center',
      paddingTop: 4,
      paddingBottom: 8,
    },
    sheetBody: {
      flex: 1,
    },
    sheetBodyContent: {
      paddingBottom: 4,
    },
    findingRow: {
      flexDirection: 'row',
      alignItems: 'flex-start',
      marginBottom: 16,
    },
    spinnerWrap: {
      width: 40,
      height: 40,
      marginRight: 12,
      marginTop: 2,
      alignItems: 'center',
      justifyContent: 'center',
    },
    spinnerRing: {
      width: 36,
      height: 36,
      borderRadius: 18,
      borderWidth: 3,
      borderColor: colors.isDark ? colors.navy.cardBg : colors.orange[200],
      borderTopColor: colors.orange[500],
      borderRightColor: colors.orange[500],
    },
    findingCopy: {
      flex: 1,
      minWidth: 0,
    },
    findingTitle: {
      fontFamily: colors.fonts.sora.extraBold,
      color: colors.text,
      fontSize: 20,
      marginBottom: 4,
    },
    findingSub: {
      fontFamily: colors.fonts.sora.medium,
      color: colors.muted,
      fontSize: 14,
      lineHeight: 20,
    },
    progressTrack: {
      height: 6,
      borderRadius: 3,
      backgroundColor: colors.border,
      overflow: 'hidden',
      marginBottom: 20,
    },
    progressFill: {
      height: '100%',
      borderRadius: 3,
      backgroundColor: colors.orange[500],
    },
    tripCard: {
      flexDirection: 'row',
      marginBottom: 16,
    },
    timeline: {
      width: 18,
      alignItems: 'center',
      marginRight: 12,
      paddingTop: 4,
      paddingBottom: 4,
    },
    pickupDot: {
      width: 12,
      height: 12,
      borderRadius: 6,
      backgroundColor: colors.orange[500],
    },
    timelineLine: {
      width: 2,
      flex: 1,
      backgroundColor: colors.border,
      marginVertical: 4,
    },
    dropSquare: {
      width: 12,
      height: 12,
      borderRadius: 3,
      backgroundColor: colors.isDark ? colors.white : colors.navy[800],
    },
    tripCopy: {
      flex: 1,
      minWidth: 0,
      gap: 16,
    },
    tripBlock: {},
    tripLabel: {
      fontFamily: colors.fonts.sora.bold,
      color: colors.muted,
      fontSize: 11,
      letterSpacing: 0.8,
      marginBottom: 4,
    },
    tripAddress: {
      fontFamily: colors.fonts.sora.bold,
      color: colors.text,
      fontSize: 15,
    },
    infoBox: {
      flexDirection: 'row',
      alignItems: 'flex-start',
      gap: 10,
      backgroundColor: colors.surface,
      borderRadius: 14,
      paddingHorizontal: 14,
      paddingVertical: 12,
      marginBottom: 16,
    },
    infoText: {
      fontFamily: colors.fonts.sora.medium,
      flex: 1,
      color: colors.muted,
      fontSize: 13,
      lineHeight: 19,
    },
    cancelBtn: {
      borderWidth: 1.5,
      borderColor: colors.isDark ? colors.alpha.red40 : colors.red.border,
      backgroundColor: colors.card,
      borderRadius: 16,
      paddingVertical: 16,
      alignItems: 'center',
      justifyContent: 'center',
    },
    cancelText: {
      fontFamily: colors.fonts.sora.extraBold,
      color: colors.red[500],
      fontSize: 16,
    },
    unavailableScroll: {
      paddingBottom: 4,
    },
    warnIconWrap: {
      width: 44,
      height: 44,
      borderRadius: 22,
      backgroundColor: colors.isDark ? colors.alpha.amberAlpha18 : colors.amber[100],
      alignItems: 'center',
      justifyContent: 'center',
      marginRight: 12,
      marginTop: 2,
    },
    altLabel: {
      fontFamily: colors.fonts.sora.bold,
      color: colors.muted,
      fontSize: 11,
      letterSpacing: 0.9,
      marginBottom: 10,
    },
    altRow: {
      flexDirection: 'row',
      gap: 12,
      marginBottom: 18,
    },
    altCard: {
      flex: 1,
      flexDirection: 'row',
      alignItems: 'center',
      borderWidth: 1.5,
      borderColor: colors.border,
      borderRadius: 18,
      paddingVertical: 14,
      paddingHorizontal: 12,
      backgroundColor: colors.card,
      gap: 8,
    },
    altName: {
      fontFamily: colors.fonts.sora.bold,
      flex: 1,
      color: colors.text,
      fontSize: 15,
    },
    altPrice: {
      fontFamily: colors.fonts.sora.extraBold,
      color: colors.orange[500],
      fontSize: 15,
    },
    searchAgainBtn: {
      backgroundColor: colors.orange[500],
      borderRadius: 16,
      paddingVertical: 16,
      alignItems: 'center',
      justifyContent: 'center',
      marginBottom: 12,
    },
    searchAgainText: {
      fontFamily: colors.fonts.sora.extraBold,
      color: colors.white,
      fontSize: 16,
    },
    cancelLink: {
      alignItems: 'center',
      paddingVertical: 6,
    },
    cancelLinkText: {
      fontFamily: colors.fonts.sora.bold,
      color: colors.muted,
      fontSize: 15,
    },
    arrivalPill: {
      position: 'absolute',
      alignSelf: 'center',
      zIndex: 7,
      flexDirection: 'row',
      alignItems: 'center',
      gap: 8,
      backgroundColor: colors.isDark ? colors.card : colors.navy[900],
      paddingHorizontal: 14,
      paddingVertical: 10,
      borderRadius: 22,
      maxWidth: '78%',
      shadowColor: colors.navy[900],
      shadowOpacity: 0.2,
      shadowRadius: 8,
      shadowOffset: {width: 0, height: 2},
      elevation: 5,
    },
    arrivalPillText: {
      fontFamily: colors.fonts.sora.bold,
      color: colors.white,
      fontSize: 13,
      flexShrink: 1,
    },
    locateFab: {
      position: 'absolute',
      right: 16,
      zIndex: 8,
      width: 48,
      height: 48,
      borderRadius: 24,
      backgroundColor: colors.card,
      alignItems: 'center',
      justifyContent: 'center',
      shadowColor: colors.navy[900],
      shadowOpacity: 0.12,
      shadowRadius: 8,
      shadowOffset: {width: 0, height: 2},
      elevation: 4,
    },
    driverRouteLine: {
      position: 'absolute',
      left: '18%',
      right: '28%',
      top: '42%',
      height: 5,
      borderRadius: 3,
      backgroundColor: colors.orange[500],
      transform: [{rotate: '-12deg'}],
    },
    driverRouteDot: {
      position: 'absolute',
      left: '16%',
      top: '40%',
      width: 12,
      height: 12,
      borderRadius: 6,
      backgroundColor: colors.orange[500],
      borderWidth: 2,
      borderColor: colors.white,
    },
    tripRouteGhost: {
      backgroundColor: colors.blue[500],
      opacity: 0.35,
      right: '12%',
    },
    tripRouteActive: {
      position: 'absolute',
      left: '18%',
      width: '28%',
      top: '42%',
      height: 5,
      borderRadius: 3,
      backgroundColor: colors.orange[500],
      transform: [{rotate: '-12deg'}],
    },
    tripCarPin: {
      position: 'absolute',
      left: '42%',
      top: '36%',
      width: 36,
      height: 36,
      borderRadius: 18,
      backgroundColor: colors.orange[500],
      alignItems: 'center',
      justifyContent: 'center',
      borderWidth: 3,
      borderColor: colors.white,
      zIndex: 4,
    },
    tripDropPin: {
      position: 'absolute',
      right: '10%',
      top: '28%',
      zIndex: 3,
    },
    navBanner: {
      position: 'absolute',
      left: 16,
      zIndex: 8,
      maxWidth: '62%',
      flexDirection: 'row',
      alignItems: 'center',
      gap: 10,
      backgroundColor: colors.isDark ? colors.card : colors.navy[900],
      paddingHorizontal: 12,
      paddingVertical: 10,
      borderRadius: 16,
    },
    navCopy: {
      flexShrink: 1,
    },
    navTitle: {
      fontFamily: colors.fonts.sora.extraBold,
      color: colors.white,
      fontSize: 13,
    },
    navSub: {
      fontFamily: colors.fonts.sora.medium,
      color: colors.isDark ? colors.muted : colors.navy[300],
      fontSize: 11,
      marginTop: 2,
    },
    sosPill: {
      position: 'absolute',
      right: 16,
      zIndex: 8,
      flexDirection: 'row',
      alignItems: 'center',
      gap: 6,
      backgroundColor: colors.card,
      borderWidth: 1.5,
      borderColor: colors.red[500],
      paddingHorizontal: 12,
      paddingVertical: 9,
      borderRadius: 20,
    },
    sosPillText: {
      fontFamily: colors.fonts.sora.extraBold,
      color: colors.red[500],
      fontSize: 13,
    },
  });
}

